...
public class CumulantAllToAllF0ODEs implements DynaComplexODEs {
     ...
     */
    public CumulantAllToAllF0ODEs(int n, double gamma, double w, double gel, DynaComplex alpha, double[] d) {
        super();
        this.n = n;
        this.gamma = gamma;
        this.w = w;
    ...
    private void initConstants() {
        ...
        for(int i = 0; i < n; ++i) {
            c1[i] = new DynaComplex(-0.5*(w + gel), -d[i]);
        }
        
        c2 = new DynaComplex[n];
        for(int i = 0; i < n; ++i) {
            c2[i] = new DynaComplex(-1.5*(w) - 0.5*gel, -d[i]);
        }
        ...
        for(int i = 0; i < n; ++i) {
            for(int j = i+1; j < n; ++j) {
                ...
                c3[i][j] = new DynaComplex(-(w + gel), -(d[i] - d[j]));
            }
        }
        ...
        for(int i = 0; i < n; ++i) {
            for(int j = i+1; j < n; ++j) {
                ...
                c4[i][j] = new DynaComplex(-(w + gel), -(d[i] + d[j]));
            }
        }
    }
    ...
    public void computeDerivatives(double t, DynaComplex[] z, DynaComplex[] zDot)
        ...
        for(int a = 0; a < n; ++a) {
            ...
            zDot[startIdx[2] + a].set(z[startIdx[2] + a]).multiply(-(w)).add(w).add(sum1);
        }
        ...
        for(int a = 0; a < n; ++a) {
            for(int b = a+1; b < n; ++b) {
                ...
                zDot[idx].add(t1.set(z[startIdx[2] + a]).add(z[startIdx[2] + b]).multiply(w));
                zDot[idx].subtract(t1.set(z[startIdx[4] + triab]).multiply(2*(w)));
                zDot[idx].add(sum1).add(sum2);
                ++idx;
            }
        }
