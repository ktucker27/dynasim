...
public class MomentsCorrelationAllToAllODEs implements DynaComplexODEs {
    
    /**
     * Fourth order central moment version of the two-time correlation solver
     */
     ...
     * @param z complex output of cumulant expansion equations
     */
    public MomentsCorrelationAllToAllODEs(int n, double gamma, double w, DynaComplex alpha, double[] d, DynaComplex[] z) {
        super();
        this.n = n;
        this.gamma = gamma;
        this.w = w;
        ...
        for(int c = 0; c < n; ++c) {
            for(int a = 0; a < n; ++a) {
                for(int b = 0; b < n; ++b) {
                    ...
                    t1.add(t2.set(ttColSums[c]).subtract(z[a*n+c]).subtract(z[b*n+c]).multiply(-1.0*szs[a]*szs[b]));
